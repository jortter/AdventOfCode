	...
	for(int j = 0; j < updates.size(); j++){	// Bucle que recorre las rules para cada update (por eso lo ponemos antes que el bucle de los updates)
		...
		do{
			...
			for(int i = 0; i < rules.size(); i++){	// Bucle que recorre los updates (filas de la matriz)
				...
				// Para esta segunda parte debemos cambiar nuestra lógica booleana al revés para operar ahora con los updates inválidos
				// Después, para reordenar los updates incorrectos para que sean correctos simplemente tenemos que hacer un swap de rules[i].first y rules[i].second mientras
				// recorremos el vector de updates
				if(pos_x != -1 && pos_y != -1){
					...
					if(pos_x >= pos_y){		
						update_valido = false;
						std::cout << "El update: "; 
						for(int n = 0; n < updates[j].size(); n++){
							std::cout << updates[j][n];
							if(n != updates[j].size()-1){
								std::cout << ',';
							}
						}
						std::cout << " NO es válido porque " << rules[i].first << " va después que " << rules[i].second <<  std::endl;	
						
						std::swap(updates[j][pos_x], updates[j][pos_y]);	
						corregido = true;
						for(int p = 0; p < updates[j].size(); p++){
							std::cout << updates[j][p];
							if(p != updates[j].size()-1){
								std::cout << ',';
							}
		...
		// Vamos haciendo swaps hasta que se cumplan todas las reglas (hasta que update_valido = true)
		}while(!update_valido);
		std::cout << "El update: ";
